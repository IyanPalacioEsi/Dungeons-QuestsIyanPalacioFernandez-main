...
public class LSPlayer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, currentPoint.transform.position) < .1f)
        {
            ...
            if (Vector3.Distance(transform.position, currentPoint.transform.position) < .1f)
            {
                //Si pulsamos el botón de movimiento vertica arriba (en el caso de un joystick a más de la mitad de su recorrido)
                ...
                //Si pulsamos el botón de movimiento horizontal a abajo (en el caso de un joystick a más de la mitad de su recorrido)
                if (Input.GetAxisRaw("Vertical") < -.5f)
                {
